...
// Function to check if coordinates are part of a grid pattern
function isGridPattern(lat, lon) {
...
async function fixTVStations() {
  try {
    ...
    for (let i = 0; i < tvStations.length; i++) {
      ...
      // Check if coordinates are placeholder values or grid pattern
      const isPlaceholderCoord = 
        ...
        isGridPattern(parseFloat(station.geo_lat), parseFloat(station.geo_long));
    ...
    console.log('\nFixed ' + stationsFixed + ' TV stations with placeholder coordinates.');
    
    // Convert to the format expected by the fixed file (latitude/longitude instead of geo_lat/geo_long)
    const fixedStations = tvStations.map(station => {
      return {
        ...station,
    ...
    await fs.writeFile('./src/data/tvStationsWithUrlsFixed.json', JSON.stringify(fixedStations, null, 2));
...
// Read and process radio stations to fix placeholder coordinates
async function fixRadioStations() {
  try {
    ...
    for (let i = 0; i < radioStations.length; i++) {
      ...
      if (isPlaceholderCoord) {
        console.log('\nFixing radio station: ' + station.name);
        console.log('  Current coords: ' + station.geo_lat + ', ' + station.geo_long);
        console.log('  Current country: ' + station.country);
    ...
    console.log('\nFixed ' + stationsFixed + ' radio stations with placeholder coordinates.');
    
    // Convert to the format expected by the fixed file (latitude/longitude instead of geo_lat/geo_long)
    const fixedStations = radioStations.map(station => {
      return {
        ...station,
        latitude: station.geo_lat,
        longitude: station.geo_long
    ...
    // Write the fixed data back to the file
    await fs.writeFile('./src/data/radioStationsFixed.json', JSON.stringify(fixedStations, null, 2));
