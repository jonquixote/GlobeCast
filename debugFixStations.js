...
async function fixTVStations() {
  try {
    ...
    for (let i = 0; i < Math.min(tvStations.length, 10); i++) {  // Process only first 10 for testing
      const station = tvStations[i];
      
      // Check if coordinates are valid
      const hasValidCoords = isValidCoordinate(station.geo_lat, station.geo_long);
      
      console.log(`Station ${i+1}: ${station.name}`);
...
async function fixRadioStations() {
  try {
    console.log('Reading radio stations file...');
    ...
    // Process each station
    for (let i = 0; i < Math.min(radioStations.length, 10); i++) {  // Process only first 10 for testing
