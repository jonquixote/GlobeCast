...
function findGridStations(filePath) {
  ...
  try {
    ...
    data.forEach((station, index) => {
      ...
      // More specific check for exact grid values that were used as placeholders
      const isGridPattern = 
        (Math.abs(lat - Math.round(lat)) < 0.001 && Math.abs(lon - Math.round(lon)) < 0.001) &&
        (lat >= 20 && lat <= 70) && 
        (lon >= -130 && lon <= -60) &&
        (
          (lat % 2 === 0 && lon % 2 === 0) ||  // Even grid
          (lat % 2 === 1 && lon % 2 === 1) ||  // Odd grid
          (lat % 2 === 0 && lon % 2 === 0)     // Mixed grid
        );
