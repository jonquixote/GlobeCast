...
// Generate 1000 diverse TV stations with real coordinates
const countries = [
...
async function generateStations() {
    ...
    for (const city of country.cities) {
      ...
      for (let i = 1; i <= 40; i++) {
        ...
        const station = {
          ...
          "latitude": coords ? coords.lat : ((Math.random() * 180) - 90), // Fallback to random if geocoding fails
          "longitude": coords ? coords.lon : ((Math.random() * 360) - 180), // Fallback to random if geocoding fails
          "categories": "general"
        };
  ...
  console.log(`Generated ${stations.length} TV stations and saved to ${outputPath}`);
}
