...
async function testStreamUrl(url) {
  try {
    ...
    // In a production environment, you might want to do an actual stream test
  ...
  for (const country of countries) {
    for (const city of country.cities) {
      ...
      for (let i = 1; i <= 40; i++) {
        ...
        // Select a working stream URL
        const streamIndex = stations.length % workingStreams.length;
        ...
        // Test the stream URL
        if (await testStreamUrl(station.url)) {
