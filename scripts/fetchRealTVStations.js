...
async function fetchRealTVStations() {
  try {
    console.log('Fetching TV stations from Free-TV/IPTV...');
    ...
    // Limit to 1000 stations and add geographic coordinates
    const limitedStations = [];
    const countryCityMap = {}; // Cache for country-city combinations
    
    for (let i = 0; i < Math.min(stations.length, 1000); i++) {
      ...
      if (coords) {
        ...
        const fallbackLat = 40.0 + (limitedStations.length % 10) * 2.0;
        const fallbackLon = -100.0 + Math.floor(limitedStations.length / 10) * 2.0;
