...
// Generate 1000 radio stations with varied coordinates
for (let i = 0; i < 1000; i++) {
  ...
  const station = {
    ...
    "url": `http://stream.radio-${radioId}.com/stream.mp3`,
    "homepage": `http://www.radio-${radioId}.com`,
    "favicon": `http://www.radio-${radioId}.com/favicon.ico`,
...
console.log(`Generated ${realisticRadioStations.length} radio stations with varied coordinates`);
...
console.log(`Generated ${realisticTVStations.length} TV stations with varied coordinates`);

// Also generate the tvStationsWithUrls.json file
const tvStationsWithUrls = realisticTVStations.map((station, index) => {
  return {
    ...station,
    url: `https://sample-streams.com/hls/tv-${index}.m3u8`
  };
});
