...
function isValidCoordinate(lat, lon) {
  ...
  const isPlaceholder =
    ...
    (lat > 20 && lat < 70 && lon > -100 && lon < -20); // Atlantic grid

  return validLat && validLon && !isPlaceholder;
}

// Function to extract location from station name
function extractLocationFromName(name) {
    // Regex for City, ST where ST is a valid US state
    const states = ["AL", "AK", "AZ", "AR", "CA", "CO", "CT", "DE", "FL", "GA",
                    "HI", "ID", "IL", "IN", "IA", "KS", "KY", "LA", "ME", "MD",
                    "MA", "MI", "MN", "MS", "MO", "MT", "NE", "NV", "NH", "NJ",
                    "NM", "NY", "NC", "ND", "OH", "OK", "OR", "PA", "RI", "SC",
                    "SD", "TN", "TX", "UT", "VT", "VA", "WA", "WV", "WI", "WY"];
    const cityStateRegex = new RegExp(`([A-Za-z\\s]+?),?\\s(${states.join('|')})\\b`);
    const match = name.match(cityStateRegex);
...
async function fixStationCoordinates(station, geocodedCache) {
  ...
  // Try to extract location from name if city is not reliable
  let extractedLocation = null;
  if (!station.city || station.city === 'Unknown' || station.city.includes(station.name.substring(0, 10))) {
      extractedLocation = extractLocationFromName(station.name);
  }
  ...
  if(extractedLocation) {
    console.log(`Extracted location for ${station.name}: ${extractedLocation.city}, ${extractedLocation.state}`);
    city = extractedLocation.city;
    state = extractedLocation.state;
  }

  // Try to geocode this location
  console.log(`Geocoding ${station.name} (${city}, ${state}, ${station.country})...`);
  const coords = await geocodeLocation(city, state, station.country);
...
async function fixAllStationCoordinates() {
  try {
    ...
    // Process TV stations
    await processStationData('./src/data/tvStationsWithUrlsFixed.json', 'TV');

    // Process radio stations
    await processStationData('./src/data/radioStationsFixed.json', 'radio');
...
async function verifyAllStationCoordinates() {
  try {
    ...
    const tvStationsData = await fs.readFile('./src/data/tvStationsWithUrlsFixed.json', 'utf8');
    ...
    const radioStationsData = await fs.readFile('./src/data/radioStationsFixed.json', 'utf8');
...
if (process.argv[1] && process.argv[1].endsWith('fixAndGeocodeStations.js')) {
